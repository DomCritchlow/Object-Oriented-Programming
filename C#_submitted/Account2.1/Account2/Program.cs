		...
		static void Main(string[] args)
		{
			...
			do
			{
				...
				case "S":
					...
					Console.Write("Enter Pin: ");
					int ServicePin = int.Parse(Console.ReadLine());

					int loopInquire=0;
					for (; loopInquire < maxAcount; loopInquire++)
					{
						if (Check[loopInquire].getPin() == ServicePin)
						{
							break;
						}
					}
					if(loopInquire==maxAcount){
						Console.Write("Wrong Code\n");
						break;
					}
					string choice2 = null;
					...
					case "D":
							...
							Console.Write("Account: {0}\n Deposite: \n", Check[loopInquire].getName());
							Check[loopInquire].deposit(double.Parse(Console.ReadLine()));
							Console.Write("Balance: {0}\n", Check[loopInquire].getBalance());
					...
					case "W":
							...
							Console.Write("Account: {0}\n Withdraw: \n", Check[loopInquire].getName());
							Check[loopInquire].withdraw(double.Parse(Console.ReadLine()));
							Console.Write("Balance: {0}\n", Check[loopInquire].getBalance());
						continue;
					}
					}while(choice2 !="R");
